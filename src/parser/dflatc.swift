  ...
  for field in structDef.fields {
        ...
        switch field.type.element!.type {
          ...
          case .union:
            ...
            for enumVal in enumDef.fields {
              ...
              code += "        guard let oe = obj.\(field.name)(at: i, type: \(GetDflatGenFullyQualifiedName(subStructDef)).self) else { break }\n"
    ...
    case .union:
      ...
      for enumVal in enumDef.fields {
        ...
        code += "    case .\(enumVal.name.lowercased()):\n"
        let subStructDef = structDefs[enumVal.struct!]!
        code += "      self.\(field.name) = obj.\(field.name)(type: \(GetDflatGenFullyQualifiedName(subStructDef)).self).map { .\(enumVal.name.firstLowercased())(\(enumVal.name)($0)) }\n"
      }
      code += "    }\n"
    case .enum:
...
func GenStructSerializer(_ structDef: Struct, code: inout String) {
  ...
  for field in structDef.fields {
    ...
    case .vector:
      if IsScalarElementType(field.type.element!) {
        ...
        case .enum:
          ...
          code += "    for i in self.\(field.name) {\n"
          code += "      __\(field.name).append(\(GetDflatGenFullyQualifiedName(enumDef))(rawValue: i.rawValue) ?? \(GetEnumDefaultValue(field.type.element!.enum!).lowercased()))\n"
          code += "    }\n"
    ...
    case .enum:
      let enumDef = enumDefs[field.type.enum!]!
      code += "    let __\(field.name) = \(GetDflatGenFullyQualifiedName(enumDef))(rawValue: self.\(field.name).rawValue) ?? \(GetFieldDefaultValue(field).lowercased())\n"
...
func GetTraverseKeyFlatBuffers(_ keyPaths: [KeyPath]) -> String {
  ...
  for (i, keyPath) in keyPaths.enumerated() {
    ...
    case .union(let field, let union):
      ...
      code += "    guard let tr\(i + 1) = tr\(i).\(field.name)(type: \(GetDflatGenFullyQualifiedName(subStructDef)).self) else { return nil }\n"
    }
  }
  return code
}
...
func GenQueryForField(_ structDef: Struct, keyPaths: [KeyPath], field: Field, pkCount: inout Int, code: inout String, addon: inout String) {
  ...
  if structDef.namespace.count > 0 {
    structProtocolName = "zzz_DflatGen_Proto__" + structDef.namespace.joined(separator: "__") + "__" + structDef.name
  } else {
  ...
  case .union:
    ...
    code += "    let tr0 = \(GetDflatGenFullyQualifiedName(structDef)).getRootAs\(structDef.name)(bb: table)\n"
    ...
    code += "    }\n"
    code += "  }\n"
    code += "  public static let _type: FieldExpr<Int32, \(GetFullyQualifiedName(structDef))> = FieldExpr(name: \"\(expandedName)__type\", primaryKey: \(field.isPrimary ? "true" : "false"), hasIndex: \(field.hasIndex ? "true" : "false"), tableReader: _tr__\(expandedName)__type, objectReader: _or__\(expandedName)__type)\n"
    code += "\n  }\n"
    ...
    for enumVal in unionDef.fields {
      ...
      addon += "  }\n"
      addon += "  public typealias zzz_AsType__\(structDef.name)__\(expandedName) = zzz_\(expandedName)__\(subStructDef.name)\n"
      addon += "\n}\n"
      addon += newAddon
    }
  case .struct:
  ...
  default: // These are the simple types (scalar) or enum
    ...
    code += "static let \(field.name): FieldExpr<\(swiftType), \(GetFullyQualifiedName(structDef))> = FieldExpr(name: \"\(key)\", primaryKey: \(field.isPrimary ? "true" : "false"), hasIndex: \(field.hasIndex ? "true" : "false"), tableReader: _tr__\(expandedName), objectReader: _or__\(expandedName))\n"
  }
}
