...
class SchemaUpgradeTests: XCTestCase {
  ...
  func testBuildIndexWithPartialIndex() {
    ...
    dflat = SQLiteWorkspace(filePath: filePath, fileProtectionLevel: .noProtection)
    let finalFetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(MyGame.Sample.Monster.mana < 120, orderBy: [MyGame.Sample.Monster.mana.ascending])
  ...
  func testUpgradeFromV1ToV2() {
    ...
    connection?.close()
    dflat = SQLiteWorkspace(filePath: filePath, fileProtectionLevel: .noProtection)
    let finalFetchedResult = dflat.fetch(for: MyGame.SampleV2.Monster.self).where(MyGame.SampleV2.Monster.mana + MyGame.SampleV2.Monster.hp > 150, orderBy: [MyGame.SampleV2.Monster.mana.descending])
    XCTAssert(finalFetchedResult.count == 2)
