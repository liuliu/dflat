...
class FetchTests: XCTestCase {
  ...
  func testFetchWithinASnapshot() {
    ...
    let fetchedResult: FetchedResult<MyGame.Sample.Monster> = dflat.fetchWithinASnapshot {
      ...
      let firstFetch = dflat.fetch(for: MyGame.Sample.Monster.self).where(
        MyGame.Sample.Monster.mana < 40)
      ...
      dflat.performChanges(
        [MyGame.Sample.Monster.self],
        changesHandler: { txnContext in
          guard let deletionRequest = MyGame.Sample.MonsterChangeRequest.deletionRequest(deletedObj)
          else { fatalError() }
      ...
      return dflat.fetch(for: MyGame.Sample.Monster.self).where(
  ...
  func testFetchWithCustomOrder1() {
    guard let dflat = dflat else { return }
    let expectation = XCTestExpectation(description: "transcation done")
    dflat.performChanges(
      [MyGame.Sample.Monster.self],
    ...
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.mana < 110,
      orderBy: [
        ["name4", "name3", "name2", "name1", "name3"].firstIndex(of: MyGame.Sample.Monster.name)
      ])
  ...
  func testFetchWithCustomOrder2() {
    guard let dflat = dflat else { return }
    let expectation = XCTestExpectation(description: "transcation done")
    dflat.performChanges(
      [MyGame.Sample.Monster.self],
    ...
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.mana < 110,
      orderBy: [["name4", "name3", "name1", "name3"].lastIndex(of: MyGame.Sample.Monster.name)])
    XCTAssert(fetchedResult.count == 3)
    // name2 is missing from the key, hence it is nil, hence it is the first.
    XCTAssertEqual(fetchedResult[0].name, "name2")
  ...
  static let allTests = [
    ...
    ("testFetchWithCustomOrder1", testFetchWithCustomOrder1),
    ("testFetchWithCustomOrder2", testFetchWithCustomOrder2),
  ]
}
