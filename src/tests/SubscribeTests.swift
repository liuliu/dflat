...
class SubscribeTests: XCTestCase {
  ...
  func testSubscribeObject() {
    ...
    dflat.performChanges(
      [MyGame.Sample.Monster.self],
      changesHandler: { (txnContext) in
        guard let deletionRequest = MyGame.Sample.MonsterChangeRequest.deletionRequest(firstMonster)
        else { return }
  ...
  static let allTests = [
    ...
    (
      "testSubscribeOutdatedObjectAndCacnelBeforeNextUpdate",
      testSubscribeOutdatedObjectAndCacnelBeforeNextUpdate
    ),
    ("testSubscribeFetchedResult", testSubscribeFetchedResult),
    (
      "testSubscribeOutdatedFetchedResultAndCancelSecondary",
      testSubscribeOutdatedFetchedResultAndCancelSecondary
    ),
  ]
