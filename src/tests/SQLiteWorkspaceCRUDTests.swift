...
class SQLiteWorkspaceCRUDTests: XCTestCase {
  ...
  func testObjectCreationAndQueryByNoneIndexedProperty() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green)
  ...
  func testSortByIndexedProperty() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).all(orderBy: [
      MyGame.Sample.Monster.mana.ascending
    ])
  ...
  func testQueryAndSortByAnotherPrimaryKey() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green, orderBy: [MyGame.Sample.Monster.mana.ascending])
    XCTAssert(fetchedResult.count == 4)
    XCTAssertEqual(fetchedResult[0].name, "name3")
    XCTAssertEqual(fetchedResult[1].name, "name2")
    XCTAssertEqual(fetchedResult[2].name, "name1")
  ...
  func testQueryAndSortByPrimaryKey() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green, orderBy: [MyGame.Sample.Monster.name.ascending])
    XCTAssert(fetchedResult.count == 4)
  ...
  func testQueryAndSortByPrimaryKeyDesc() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green, orderBy: [MyGame.Sample.Monster.name.descending])
    XCTAssert(fetchedResult.count == 4)
  ...
  func testQueryByUnionType() {
    ...
    wait(for: [expectation], timeout: 10.0)
    let fetchedResult = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.equipped.as(MyGame.Sample.Orb.self).color == .green)
    XCTAssert(fetchedResult.count == 1)
    XCTAssertEqual(fetchedResult[0].name, "name3")
    let allOrbs = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.equipped.match(MyGame.Sample.Orb.self),
      orderBy: [MyGame.Sample.Monster.equipped.as(MyGame.Sample.Orb.self).name.descending])
  ...
  func testFetchWithinATransactionToSpeedupChangeRequest() {
    ...
    wait(for: [expectation1], timeout: 10.0)
    let fetchedResult1 = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green)
    ...
    XCTAssertEqual(
      changeRequestRetrieved, 4, "should be able to get 4 change requests in the second transaction"
    )
    let fetchedResult2 = dflat.fetch(for: MyGame.Sample.Monster.self).where(
      MyGame.Sample.Monster.color == .green)
    for monster in fetchedResult2 {
      XCTAssertEqual(
        monster.mana, 110, "mana from the second change request should be the updated value")
    }
  }
  ...
  static let allTests = [
    ...
    (
      "testObjectCreationAndQueryByNoneIndexedProperty",
      testObjectCreationAndQueryByNoneIndexedProperty
    ),
    ...
    (
      "testFetchWithinATransactionToSpeedupChangeRequest",
      testFetchWithinATransactionToSpeedupChangeRequest
    ),
